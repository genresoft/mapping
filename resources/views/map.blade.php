        ...
        <div class="col-md-12">
            <div class="content-box content-single">
                <article class="post-180 gd_place type-gd_place status-publish hentry gd_placecategory-hotels">
                    <header>
                        <h1 class="entry-title">{{ $map->name }}</h1></header>
                                            ...
                                            <div class="geodir_post_meta  geodir-field-post_content">
                                                ...
                                                @if($map->days->count())
                                                    @if($map->working_hours->currentOpenRange(now()))
                                                        <p>Village office is open and will close at {{ $map->working_hours->currentOpenRange(now())->end() }}.</p>
                                                    @else
                                                        <p>Village office is closed since {{ $map->working_hours->previousClose(now())->format('l H:i') }}
                                        ...
                                        <li>
                                            <div class="geodir-post-title">
                                                <h4 class="geodir-entry-title">
                                                    <a href="{{ route('map', $map->id) }}" title="View: {{ $map->name }}">{{ $map->name }}</a>
                                                </h4>
                                            </div>
                                            <a href="{{ route('map', $map->id) }}"><img src="{{ $map->thumbnail }}" alt="{{ $map->name }}" class="align size-medium_large" width="1400" height="930"></a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                ...
                <div class="geodir-bubble-meta-side">
                    ...
                    <div class="geodir-output-location geodir-output-location-mapbubble">
                        <div class="geodir_post_meta  geodir-field-post_title"><span class="geodir_post_meta_icon geodir-i-text">
                            ...
                            <span class="geodir_post_meta_title">Place Title: </span></span>{{ $map->name }}</div>
                        <div class="geodir_post_meta  geodir-field-address" itemscope="" itemtype="http://schema.org/PostalAddress">
                            ...
                            <span class="geodir_post_meta_title">Address: </span></span><span itemprop="streetAddress">{{ $map->address }}</span>
                        </div>
                    </div>
                    </div>
                </div>
