...
class MapsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $addresses = [
            ...
            [
                "address" => "The Harry Potter Map at Platform 9 3/4, Pancras Road, London",
                "latitude" => "51.5321845",
        ...
        foreach($users as $user)
        {
            ...
            $map = $user->maps()->create(array_merge($map, $addresses[$currentAddress++]));
            $map->categories()->sync($categories->random(rand(0,3)));

            foreach($pictures->random(rand(1,3)) as $index)
            {
                $map->addMedia(public_path("assets/images/maps/a$index.jpg"))->preservingOriginal()->toMediaCollection('photos');
            }
        }
    }
}
